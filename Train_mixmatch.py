...
def warmup(epoch, net, optimizer, dataloader):
    ...
    for batch_idx, (inputs, labels, _) in enumerate(dataloader):
        ...
        with torch.cuda.amp.autocast():
            ...
            weight = np.exp(label_dist.log_prob(torch.tensor([0.1123]))) - weight
...
if __name__ == '__main__':
    ...
    for epoch in range(args.num_epochs + 1):
        ...
        if epoch < warm_up:
            ...
            train(epoch=epoch, net=net, optimizer=optimizer, labeled_trainloader=labeled_trainloader, unlabeled_trainloader=unlabeled_trainloader)
        test(epoch, net)
